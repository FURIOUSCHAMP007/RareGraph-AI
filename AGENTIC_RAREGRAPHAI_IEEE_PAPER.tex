...
\end{equation}
The expected information gain $\operatorname{IG}(T_k)$ for an unperformed diagnostic test $T_k$ is computed as:
\begin{equation}
\operatorname{IG}(T_k) = H(D) - \sum_{j \in \text{Outcomes}(T_k)} P(t_{k,j}) H(D \mid t_{k,j})
\end{equation}
The system prioritizes clinical tests that maximize $\operatorname{IG}(T_k)$, directing clinicians toward investigations with the highest explanatory power \cite{ref10}.
...
\begin{equation}
\operatorname{Sim}(P_1, P_2) = \frac{\sum_{h_1 \in P_1} \max_{h_2 \in P_2} \operatorname{IC}(\operatorname{MICA}(h_1, h_2))}{\lvert P_1 \cup P_2 \rvert}
\end{equation}
where $\operatorname{MICA}(h_1, h_2)$ represents the Most Informative Common Ancestor in the HPO ontological tree, and the Information Content $\operatorname{IC}(h)$ is defined as:
\begin{equation}
\operatorname{IC}(h) = -\log_2 P(h)
\end{equation}
